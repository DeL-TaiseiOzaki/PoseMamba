...
def gen_video_kpts(video, det_dim=416, num_peroson=1, gen_output=False):
    ...
    for ii in tqdm(range(video_length)):
        ...
        if people_track.shape[0] == 1:
            ...
            # No person detected, append zero arrays to maintain consistent dimensions
    ...
    # Handle case where no valid frames were detected
    if len(kpts_result) == 0:
        raise ValueError("No keypoints detected in the video. Please check the video file and ensure people are visible.")

    if len(keypoints.shape) < 4:
        raise ValueError(f"Unexpected keypoints shape {keypoints.shape}, expected 4D array (T, M, N, 2). "
                        f"This may indicate an issue with the video processing.")
